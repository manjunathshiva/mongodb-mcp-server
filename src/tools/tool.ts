...
> {
            ...
            if (this.requiresConfirmation()) {
                ...
                if (!confirmation.ok) {
                    if (confirmation.reason === "no-elicitation-support") {
                        // OWASP MCP06: refuse the destructive op when the
                        // client cannot show a confirmation prompt. The user
                        // never had the chance to consent, so silently
                        // proceeding (the pre-Phase-C behaviour) would be a
                        // policy violation.
     ...
     * decline ("user clicked no") from an environment-level inability to
     * obtain consent ("client doesn't support elicitation"). Both block
     * execution, but the two cases want different error messages and
     * different audit-log severities.
     *
     * @param args - The tool arguments
     */
