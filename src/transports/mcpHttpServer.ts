...
> extends ExpressBasedHttpServer {
    ...
    protected setupMiddlewares(): void {
        ...
        // isn't, the server only accepts traffic on loopback (enforced at
        // startup by StreamableHttpRunner.validateConfig).
        if (this.userConfig.oauthIssuer && this.userConfig.oauthAudience) {
            ...
            this.logger.info({
                ...
                context: "mcpHttpServer",
                message: "OAuth bearer-token authentication is NOT enabled. The HTTP transport accepts unauthenticated connections; this is only safe on a loopback bind.",
                noRedaction: true,
            });
        }
