 ...
 * middleware. Tools and downstream middleware can read `req.auth` to
 * find out who is making the call. The shape is intentionally narrow:
 * we surface only the subject and the granted scopes; the raw token
 * is never exposed past the middleware so it cannot accidentally land
 * in logs or telemetry.
 */
export interface AuthContext {
    /** RFC 7519 `sub` claim — stable principal identifier from the IdP. */
    sub: string;
    /** Parsed RFC 8693 `scope` claim (space-delimited string → array). */
    scopes: string[];
...
/**
 * Helper: read the AuthContext attached by the middleware. Express's
 * Request type isn't augmented globally (that requires
 * @types/express-serve-static-core which we don't ship), so reads go
 * through this typed accessor. Returns undefined when OAuth is disabled
 * (loopback bind without oauthIssuer).
 */
 ...
 * header. The `error` and `error_description` parameters follow RFC 6750
 * §3 so well-behaved clients can react programmatically (and so
 * security scanners can grade us correctly).
 */
function send401(
    res: Response,
 ...
 *     cached for {@link OAuthMiddlewareOptions} jwksCache TTL)
...
export function createOAuthMiddleware({ issuer, audience, jwksCache, logger }: OAuthMiddlewareOptions) {
    ...
    return async function oauthMiddleware(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        try {
            ...
            // Discovery failure was already logged at error level inside the cache.
            // Surface 503 because the *server* couldn't reach the issuer — this is
            // not a client error and clients should retry.
            res.status(503).json({
                jsonrpc: "2.0",
                error: {
                    code: -32001,
        ...
        // oauth4webapi.validateJwtAccessToken takes a Request object so it can
        // inspect the Authorization header itself. Construct a minimal Request
        // wrapping just the header to avoid forwarding the full Express body.
        ...
        // RFC 8693 `scope` claim is a space-delimited string. Some IdPs use
        // the `scp` array form (Azure AD); accept both.
