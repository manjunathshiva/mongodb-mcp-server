...
// OAuth + non-loopback bind config validation. The runner runs the check
// inside start(); validation failures throw before any socket is opened
// so each test is cheap.
describe("StreamableHttpRunner: OAuth config validation", () => {
    ...
    it("permits loopback bind without OAuth (default test config)", async () => {
        // We don't actually want to bring up a socket here; we just want
        // validation to pass without throwing. The actual start() will
        // open a port; we close it immediately.
        runner = new StreamableHttpRunner({
