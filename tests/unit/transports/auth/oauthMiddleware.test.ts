...
// Mock oauth4webapi so the middleware can be unit-tested without real JWTs,
// real JWKS, or real network access. Each test below drives the mock to
// the outcome it wants to verify (signature OK, signature bad, etc.).
...
describe("createOAuthMiddleware", () => {
    ...
    it("returns 401 when Authorization header is not a Bearer scheme", async () => {
        ...
        expect(next).not.toHaveBeenCalled();
        expect(statusSpy).toHaveBeenCalledWith(401);
        expect(setHeaderSpy).toHaveBeenCalledWith("WWW-Authenticate", expect.stringContaining('error="invalid_request"'));
    });
